...
class MeowBottomNavigation : FrameLayout {
    ...
    init {
        val spacingDefault = resources.getDimension(R.dimen.size_tool_bar).toInt()
        heightCell = spacingDefault.dp(context)
    }
    ...
    fun add(model: Model) {
        ...
        cell.apply {
            ...
            layoutParams = params
            icon = model.icon
            iconUnSelect = model.iconUnSelect
            title = model.title
            count = model.count
    ...
    class Model(var id: Int, var title: String, var icon: Int, var iconUnSelect: Int) {
