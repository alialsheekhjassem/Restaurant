package magma.abikarshak.restaurant.utils.meow_bottom_navigation
...
import magma.abikarshak.restaurant.databinding.MeowNavigationCellBinding
...
class MeowBottomNavigationCell @JvmOverloads constructor(
    ...
    private var progress = 0f
        set(value) {
            ...
            if (isEnabledCell) {
                binding.txtTitle.setPadding(0, 15, 0, 0)
                binding.iv.setImageResource(icon)
            } else {
                binding.txtTitle.setPadding(0, 100, 0, 0)
                binding.iv.setImageResource(iconUnSelect)
            }
